<?php
require_once "WireException.php";
require_once "WireConfigurationException.php";
...
class WireFactory {
	...
	function getInstance($class) {
		...
		if(isset($config)) {
			...
			} catch(WireConfigurationException $ex) {
				throw new WireException("Bean '" . htmlspecialchars($id) . "' is not correctly defined! " . $ex->getMessage(), NULL, $ex);
			}
		}
		throw new WireException("Bean '" . htmlspecialchars($id) . "' is not defined!");
	}
	...
	private function createInstance($config) {
		...
		if(!isset($config["class"])) {
			throw new WireConfigurationException("Configuration is missing the class attribute!");
		}
		...
		if(isset($config["args"])) {
			if(!is_array($config["args"])) {
				throw new WireConfigurationException("The configuration args attribute should be an array!");
			}
			foreach($config["args"] as $arg) {
				if(!is_array($arg)) {
					throw new WireConfigurationException("The constructor arg definition should be an array of type => value!");
				}
				if(isset($arg["class"])) {
					...
					throw new WireConfigurationException("Invalid constructor arg definition! Type should be either bean or value!");
				}
			}
		}
